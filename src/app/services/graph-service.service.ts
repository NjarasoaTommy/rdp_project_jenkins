...
export class GraphService {
  ...
  showError = false;
  showErrorSubject = new Subject<boolean>();
  ...
  emitNodesSubject() {
    this.nodesSubject.next(this.nodes.slice());
  }
  ...
  checkError() {
    ...
    const allPlaceWithError = Array.from(
      this.host.querySelectorAll('div.error')
    ) as HTMLElement[];
  ...
  removeError() {
    ...
    const allLabelWithError = Array.from(
      this.host.querySelectorAll('strong.error')
    ) as HTMLElement[];
